...
viz <- animint(
  ...
  penalty=ggplot()+
    ...
    # Shorter panel stack (matches coverage height) — avoids an overly tall penalty grid
    theme_animint(width=800, height=500)+
    facet_grid(. ~ what, scales="free")+
    ggtitle("Select sample and number of peaks")+
    geom_text(aes(x, log.penalty, label=label, hjust=hjust),
              data=data.table(
                what="regression",
                x=c(6.2, Inf, Inf),
                log.penalty=c(13.0, 10.2, 9.0),
                hjust=c(1, 1, 1),
